package com.example.helios.draglistview.view;
...
/**
 * Created by Bonus Liu on 1/21/16- 4:34 PM
...
public class DragView extends ListView {
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (ev.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            // 拿到拖动的imageview对象
            View drager = itemView.findViewById(R.id.imageView);

            // 判断条件为拖动touch图片是否为null和touch的位置，是否符合 xushiyong : 添加额外的判断x < 200
            // ,用来解除整个item被拖拽事件占据,无法实现长点击事件
            if (drager != null && x > drager.getLeft() - 20 && x < 150) {
    ...
    public void startDrag(Bitmap bm, int y) {
        ...
        layoutParams.screenBrightness = 30;
        // 设置动画
        layoutParams.windowAnimations = R.style.Drag_animation;
    ...
    public void onDrop(int y) {
        ...
        // 数据交换--此处可以使用数据库的形式进行数据存储，
        if (dragPosition >= 0 && dragPosition < getAdapter().getCount()) {
            final DragListViewAdapter adapter = (DragListViewAdapter) getAdapter();
            Object srcItem = adapter.getItem(dragSrcPosition);
            Object dragItem = adapter.getItem(dragPosition);
            adapter.remove(srcItem);
            adapter.insert(srcItem, dragPosition);
            adapter.remove(dragItem);
            adapter.insert(dragItem, dragSrcPosition);
